...
@Component({
  ...
  template: `
    <div class="space-y-6">
      ...
      <!-- Pool info card -->
      <div class="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl shadow-md border border-purple-100 p-6">
        <div class="flex flex-col sm:flex-row items-start justify-between gap-4">
          <div class="flex-1">
            <h2 class="text-2xl mb-2 text-gray-800">{{ data.pool.name }}</h2>
            <div class="flex items-center gap-4 text-sm text-gray-600">
              <div class="flex items-center gap-1">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/>
                </svg>
                {{ data.pool.members.length }} miembros
              </div>
              <div class="flex items-center gap-1">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <div *ngIf="activeTab() === 'rankings'" class="space-y-4">
        <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
          ...
          <div class="divide-y divide-gray-100">
            <div *ngFor="let ranking of data.pool.rankings; let i = index"
              [class]="'p-6 flex items-center gap-4 ' + (ranking.userId === data.currentUser.id ? 'bg-green-50' : 'hover:bg-gray-50 transition-colors')">
              ...
              <!-- Nombre -->
              <div class="flex-1">
                <h4 class="text-lg text-gray-800">{{ ranking.userName }}</h4>
                <span *ngIf="ranking.userId === data.currentUser.id"
                  class="px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-xs">Tú</span>
              </div>

              <!-- Puntos -->
              <div class="text-right">
                <div class="text-3xl text-gray-800">{{ ranking.totalPoints }}</div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div class="bg-white rounded-xl shadow-md border border-gray-100 p-6 text-center">
            <div class="text-3xl text-gray-800 mb-2">45</div>
            <div class="text-sm text-gray-500">Total de puntos</div>
          </div>
          <div class="bg-white rounded-xl shadow-md border border-gray-100 p-6 text-center">
            <div class="text-3xl text-gray-800 mb-2">12</div>
            <div class="text-sm text-gray-500">Predicciones correctas</div>
          </div>
          <div class="bg-white rounded-xl shadow-md border border-gray-100 p-6 text-center">
            <div class="text-3xl text-gray-800 mb-2">8</div>
      ...
      <div *ngIf="activeTab() === 'my-pools'" class="grid gap-4">
        <div class="bg-white rounded-xl shadow-md border border-gray-100 p-6 hover:shadow-xl transition-all duration-300 cursor-pointer">
          <div class="flex items-start justify-between gap-4">
            <div class="flex-1">
              <h3 class="text-xl mb-2 text-gray-800">{{ data.pool.name }}</h3>
              ...
              <div class="flex items-center gap-2">
                ...
                <code class="px-2 py-1 bg-gray-100 rounded text-sm">{{ data.pool.code }}</code>
              </div>
            </div>
            <button (click)="activeTab.set('rankings')"
...
export class PoolsComponent {
  ...
  get scheduledMatches() { return this.data.matches.filter(m => m.status === 'scheduled'); }
  get finishedMatches()  { return this.data.matches.filter(m => m.status === 'finished'); }
  ...
  savePred(matchId: string) {
    this.getPred(matchId).saved = true;
  ...
  copyCode() {
    navigator.clipboard.writeText(this.data.pool.code).catch(() => {});
