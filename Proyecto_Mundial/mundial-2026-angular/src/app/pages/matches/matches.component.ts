      ...
      <div class="bg-white rounded-xl shadow-md border border-gray-100 p-8">
        ...
        <div class="flex items-center justify-between mb-8 max-w-3xl mx-auto">
          ...
          <div class="text-center px-8">
            <div *ngIf="matchDetail.status === 'finalizado' || matchDetail.status === 'en_curso'"
              class="text-6xl text-gray-800">
              {{ matchDetail.homeScore }} - {{ matchDetail.awayScore }}
            </div>
            <ng-container *ngIf="matchDetail.status === 'programado'">
              ...
              <div class="flex flex-col lg:flex-row justify-between gap-4">
                <div class="flex-1">
                  ...
                  <div class="flex items-center gap-2 mb-4 flex-wrap">
                    <span *ngIf="match.status === 'en_curso'"
                      class="flex items-center gap-1 px-3 py-1 bg-red-100 text-red-700 rounded-full text-sm">
                      <span class="w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>
                      EN VIVO
                    </span>
                    <span *ngIf="match.status === 'programado'"
                      class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm">PROGRAMADO</span>
                    <span *ngIf="match.status === 'finalizado'"
                  ...
                  <div class="flex items-center justify-between max-w-2xl">
                    ...
                    <div class="text-center px-6">
                      <span *ngIf="match.status === 'finalizado' || match.status === 'en_curso'"
...
export class MatchesComponent {
  ...
  get matchDetail(): any | null {
    const id = this.route.snapshot.paramMap.get('id');
    if (!id) return null;
    return this.allMatches().find((m: any) => m.id == id || m.id === id) ?? null;
  }

  get filteredMatches(): any[] {
    const f = this.filter();
    if (f === 'all') return this.allMatches();
    return this.allMatches().filter(m => m.status === f);
  }

  get liveCount()      { return this.allMatches().filter((m: any) => m.status === 'en_curso').length; }
  get scheduledCount() { return this.allMatches().filter((m: any) => m.status === 'programado').length; }
  get finishedCount()  { return this.allMatches().filter((m: any) => m.status === 'finalizado').length; }

  get filterButtons() {
    return [
      { value: 'all',       label: `Todos (${this.allMatches().length})` },
      { value: 'en_curso',      label: '🔴 En Vivo' },
