...
@Component({
  ...
  template: `
    <div class="space-y-8">
      ...
      <!-- ── Header ── -->
      <div>
        <h1 class="text-3xl mb-2 text-gray-900">¡Bienvenido, {{ currentUser()?.name }}! ⚽</h1>
      ...
      <div class="col-span-1 lg:col-span-2">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl text-gray-800 flex items-center gap-2">
            Partido Destacado <span class="text-xl">🌟</span>
          </h2>
        </div>

        <div *ngIf="liveMatch()" class="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-sm border border-gray-100 p-8 relative overflow-hidden">
          <div class="absolute top-0 right-0 w-32 h-32 bg-red-500 rounded-bl-full opacity-5 pointer-events-none"></div>
        ...
        <div *ngIf="!liveMatch()" class="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-sm border border-gray-100 p-12 text-center text-gray-500">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <app-card className="p-6">
          ...
          <div class="space-y-4">
            <div *ngFor="let notif of recentNotifications()" class="p-4 bg-gray-50 rounded-lg flex gap-4 items-start">
...
export class DashboardComponent {
  ...
  liveMatch = signal<any | undefined>(undefined);
  ...
  loadData() {
    this.partidosService.getAll().subscribe(matches => {
      ...
      this.stats.update((s: any[]) => { s[0].value = matches.filter(m => m.status === 'programado').length.toString(); return s; });
    });

    this.notificacionesService.getAll().subscribe(notifs => {
      this.recentNotifications.set(notifs.slice(0, 3));
      this.stats.update((s: any[]) => { s[3].value = notifs.filter(n => !n.read).length.toString(); return s; });
    });

    this.entradasService.getAll().subscribe(tickets => {
      this.stats.update((s: any[]) => { s[2].value = tickets.length.toString(); return s; });
    });
  }
