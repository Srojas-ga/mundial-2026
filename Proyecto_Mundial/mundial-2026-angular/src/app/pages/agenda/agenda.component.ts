        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select [(ngModel)]="selectedTeam"
              ...
              <option *ngFor="let team of teams()" [value]="team.id || team.name">{{ team.flag }} {{ team.name }}</option>
            </select>
          </div>
        </div>
...
export class AgendaComponent {
  ...
  loadData() {
    this.partidosService.getAll().subscribe(data => {
      ...
      data.forEach((m: any) => {
        if (m.homeTeam) t.set(m.homeTeam.id || m.homeTeam.name, m.homeTeam);
        if (m.awayTeam) t.set(m.awayTeam.id || m.awayTeam.name, m.awayTeam);
      });
  ...
  get filteredMatches(): any[] {
    return this.matches().filter(match => {
      if (this.selectedCity !== 'all' && (match.city || match.stadium) !== this.selectedCity) return false;
      if (this.selectedTeam !== 'all') {
        const hId = match.homeTeam?.id || match.homeTeam?.name;
        const aId = match.awayTeam?.id || match.awayTeam?.name;
        if (hId !== this.selectedTeam && aId !== this.selectedTeam) return false;
      }
      return true;
    });
  }

  get favoriteMatches(): any[] {
    const fav = this.currentUser?.favoriteTeams || [];
    return this.matches().filter(m =>
      fav.includes(m.homeTeam?.id) || fav.includes(m.awayTeam?.id)
    );
  }

  get matchesByDate(): { date: string; matches: any[] }[] {
  ...
  statusClass(status: string): string {
    ...
    if (status === 'en_curso')   return `${base} bg-red-100 text-red-700`;
    if (status === 'programado') return `${base} bg-blue-100 text-blue-700`;
